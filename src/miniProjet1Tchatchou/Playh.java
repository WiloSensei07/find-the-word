...
public class Playh extends BasicGameState {

	public String mouse;
	Image image, image1, image3, image4, image5;
	int imageX, imageY;
	int xpos, ypos;
	ArrayList<Ennemi> En;
	float compt;
	...
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
		...
		image1 = new Image("ressource/fond5.jpg");
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException {
		...
		for (int i = 0; i < En.size(); i++) {
			...
			En.get(i).deplacer(delta * 30); // modifier le *20 pour les niveaux supérieurs
		...
		if (valider) {
			...
			for (int i = 0; i < En.size(); i++) {
				...
				if (En.get(i).getLettres().equals(mot)) {
					// r.play();
					En.remove(i);
					mot = "";
					scr += 20;
